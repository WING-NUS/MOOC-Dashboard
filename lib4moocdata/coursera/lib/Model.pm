...
sub Getthreadids{
	...
	# my $query = "select id,docid,courseid,instReplied,title,posted_time from thread where courseid=? ";
	my $query = "select id,docid,courseid,inst_replied,title,posted_time from thread_new where courseid=? ";
...
sub getthread{
	...
	my $row = $dbh->selectrow_arrayref("select courseid, id, forumid from thread_new where docid = $docid");
	if (!defined $row){
...
sub getThreadtype{
	...
	my $query = "select forumname from forum
				where id = (select forumid from thread_new where docid = $docid)
				and courseid = (select courseid from thread_new where docid = $docid)";
...
sub hasInstReplied{
	my($dbh, $docid) = @_;
	# my $query = "select instReplied from thread where docid = $docid";
	my $query = "select inst_replied from thread_new where docid = $docid";
	my $instReplied = @{$dbh->selectcol_arrayref($query)}[0];
	return $instReplied;
}
...
sub updateInterventionDensity{
	...
	my $threadqry = "select count(id) from thread_new where courseid = ? and forumid = ?";
